...
use rosc::OscArray as RoscArray;
use rosc::OscMessage as RoscMessage;
use rosc::OscPacket as RoscPacket;
use rosc::OscType as RoscType;
...
impl Osc {
        ...
        std::thread::spawn(move || {
            ...
            loop {
                let size = match recv_sock.recv_from(&mut buf) {
                    Ok((size, _addr)) => size,
                    Err(e) => {
                        log::error!("Failed to receive on OSC socket: {e}");
                        continue;
                    }
                };
                ...
                let packet = match rosc::decoder::decode(&buf[..size]) {
                    ...
                    Err(e) => {
                        log::error!("Failed to parse OSC packet: {e}");
                        continue;
                    }
                };
    ...
    pub fn send(&mut self, dest: &SocketAddr, addr: String, args: Vec<OscType>) {
        ...
        if let Err(e) = self.send_sock.send_to(&buf, dest) {
            log::error!("Failed to send OSC to {dest}: {e}");
        }
    }
}
